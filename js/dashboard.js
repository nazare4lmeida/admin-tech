...
(function () {
    ...
    items: [
      {
        ...
        descricao: "Email e senha estão em Base64 no auth.js. Base64 não é criptografia — qualquer pessoa com acesso ao JS pode decodificar.",
        recomendacao: "Mover autenticação para o Supabase Auth (email + senha com bcrypt). Remover credenciais hardcoded do frontend.",
      },
      {
        status: "warn",
        titulo: "Sessão armazenada em localStorage",
        descricao: "A sessão usa localStorage com hash simples. Vulnerável a XSS: um script malicioso pode roubar o token.",
        recomendacao: "Usar cookies httpOnly para sessão, ou adotar Supabase Auth que gerencia tokens com segurança.",
      },
      {
        status: "ok",
        titulo: "Proteção contra SQL Injection",
        descricao: "O sistema usa a API REST do Supabase com parâmetros URL — não concatena SQL diretamente. Risco baixo.",
        recomendacao: "Manter o padrão atual. Nunca construir queries SQL via concatenação de strings.",
      },
      {
        status: "ok",
        titulo: "Proteção contra XSS",
        descricao: "Os dados exibidos em tabela usam .textContent e createElement, não innerHTML com dados do usuário.",
        recomendacao: "Continuar evitando innerHTML com dados externos. Revisar templates HTML que interpolam dados do banco.",
      },
      {
        status: "ok",
        titulo: "Chave anon do Supabase exposta",
        descricao: "A anon key é pública por design no Supabase — é seguro expô-la. As RLS policies protegem os dados.",
        recomendacao: "Verificar se todas as tabelas têm RLS habilitado. Nunca expor a service_role key no frontend.",
      },
      {
        status: "info",
        titulo: "Sem controle de roles/permissões",
        descricao: "Existe apenas um nível de acesso (admin). Não há distinção entre leitura e escrita por usuário.",
        recomendacao: "Para múltiplos admins, implementar roles no Supabase Auth com políticas RLS diferenciadas.",
      },
      {
        status: "info",
        titulo: "Sem timeout de sessão",
        descricao: "A sessão não expira automaticamente. Uma vez logado, o acesso persiste indefinidamente.",
        recomendacao: "Adicionar verificação de timestamp na sessão e forçar relogin após N horas de inatividade.",
      },
      {
        status: "ok",
        titulo: "HTTPS em produção (Vercel/Netlify)",
        descricao: "Deploy em Vercel/Netlify garante HTTPS automático, protegendo dados em trânsito.",
  ...
  async function render() {
    ...
    for (const f of GT.FORMATIONS) {
      try {
        ...
        metricas[f.id] = { label: f.label, icon: f.icon, total: students.length, students };
        totalAlunos += students.length;
      } catch {
        metricas[f.id] = { label: f.label, icon: f.icon, total: 0, students: [] };
      }
    }

    // Conta aprovados total
    let totalAprovados = 0, totalParticipacao = 0, totalVinculacao = 0;
    ...
    <div style="padding:0 0 40px">
      ...
      <div class="rpt-section" style="margin-bottom:24px">
        ...
        <div style="display:grid;grid-template-columns:repeat(auto-fit,minmax(180px,1fr));gap:10px;margin-top:12px">
          ${[
            ...
            { icon: "🔒", label: "Auditoria de Segurança", id: "btnSecurityAudit" },
          ].map(t => `
      ...
      <!-- Auditoria de Segurança -->
      <div class="rpt-section" id="btnSecurityAudit">
        <h2 class="rpt-exec-title">🔒 Auditoria de Segurança</h2>
        <p style="font-size:13px;color:var(--text3);margin-bottom:16px">
          Análise do sistema de autenticação e proteções implementadas.
        </p>
        <div style="display:flex;flex-direction:column;gap:10px">
          ${SECURITY_AUDIT.items.map(item => {
            const colors = {
              ok:   { bg: "rgba(38,182,103,0.07)",  border: "rgba(38,182,103,0.25)",  icon: "✅", label: "OK" },
              warn: { bg: "rgba(239,159,39,0.07)",   border: "rgba(239,159,39,0.25)",  icon: "⚠️", label: "Atenção" },
              info: { bg: "rgba(55,104,219,0.07)",   border: "rgba(55,104,219,0.25)",  icon: "ℹ️", label: "Info" },
            };
            ...
            <div style="background:${c.bg};border:1px solid ${c.border};border-radius:10px;padding:14px 16px">
              <div style="display:flex;align-items:center;gap:8px;margin-bottom:6px">
                <span>${c.icon}</span>
                <strong style="font-size:13px;color:var(--text1)">${item.titulo}</strong>
                <span style="font-size:11px;font-weight:600;padding:2px 8px;border-radius:20px;background:${c.border};color:var(--text1);margin-left:auto">${c.label}</span>
              </div>
              <p style="font-size:12px;color:var(--text2);margin-bottom:6px;line-height:1.5">${item.descricao}</p>
              <p style="font-size:12px;color:var(--text3);line-height:1.5">💡 ${item.recomendacao}</p>
            </div>`;
          }).join("")}
        </div>
      </div>
    ...
    // Wire theme toggle
    document.getElementById("themeToggleDashboard")?.addEventListener("click", () => {
  ...
  window.showTutorialStep = function(i) {
    document.getElementById("tutorialContent").innerHTML = renderTutorialStep(i);
